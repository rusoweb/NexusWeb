 ...
 */
function renderSpacingValue(value) {
 ...
 */
export function compileInlineStyle(style) {
    const parts = [];
    for (const [key, value] of Object.entries(style)) {
        ...
        if ((key === "padding" || key === "margin") && typeof value === "object") {
            parts.push(`${cssKey}: ${renderSpacingValue(value)}`);
        }
        else {
 ...
 */
function breakpointMediaQuery(bp) {
    switch (bp) {
        case "sm":
 ...
 */
export function compileResponsiveStyles(nodeId, responsive) {
    if (!responsive)
        return "";
    const blocks = [];
    ...
    for (const bp of breakpoints) {
        ...
        if (!mediaQuery)
            continue;
        const inlineStyle = compileInlineStyle(styles);
        if (!inlineStyle)
            continue;
 ...
 * - Global style tokens
 */
export function compileGlobalCSS(scene) {
    const parts = [];
    parts.push("<style>");
    // CSS reset
    parts.push(`*,
