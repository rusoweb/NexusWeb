import type { NexusScene, NexusNode } from "@nexusweb/core";
...
function getNodeTag(node: NexusNode): string {
  if (node.tag) return node.tag;
  const defaultTag =
    NODE_TYPE_DEFAULT_TAG[node.type as keyof typeof NODE_TYPE_DEFAULT_TAG];
...
function buildAttributes(
  ...
  if (tag === "img") {
    ...
    if (node.data.alt) attrs.push('alt="' + escapeHtml(node.data.alt) + '"');
    else attrs.push('alt=""');
  }
  ...
  if (tag === "a") {
    if (node.data.href) attrs.push('href="' + escapeHtml(node.data.href) + '"');
  ...
  if (tag === "input") {
    ...
    if (node.data.name) attrs.push('name="' + escapeHtml(node.data.name) + '"');
    if (node.data.value)
    ...
    if (node.data.required) attrs.push("required");
    if (node.data.disabled) attrs.push("disabled");
  }

  if (tag === "video") {
    if (node.data.autoplay) attrs.push("autoplay");
    if (node.data.muted) attrs.push("muted");
    if (node.data.loop) attrs.push("loop");
    if (node.data.assetRef)
      attrs.push('src="' + escapeHtml(node.data.assetRef) + '"');
  }
...
function renderNode(node: NexusNode, nodes: Record<string, NexusNode>): string {
  const tag = getNodeTag(node);
...
export function generateHTML(scene: NexusScene): string {
  ...
  headParts.push(
    "<title>" + escapeHtml(scene.meta.title || scene.name) + "</title>",
  );
  ...
  if (scene.meta.favicon) {
    headParts.push(
      '<link rel="icon" href="' + escapeHtml(scene.meta.favicon) + '" />',
    );
  }
