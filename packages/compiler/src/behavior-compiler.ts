...
function compileOnClickHandler(
  nodeId: string,
  behavior: OnClickBehavior,
): string {
 ...
 */
export function compileBehaviors(scene: NexusScene): string {
  const allNodes = flattenTree(scene.rootId, scene.nodes);
  const handlerEntries: string[] = [];

  for (const node of allNodes) {
    const behavior = node.data?.behavior as NodeBehavior | undefined;
    if (!behavior?.onClick) continue;
